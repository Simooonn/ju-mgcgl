...
class WordsSafe
{
    ...
    public function __construct()
    {
        ...
        foreach ($this->_mgclx as $mgc_lx) {
            $words = $this->sensitive_words($mgc_lx);
            if(!empty($words)){
                $this->load($words,$mgc_lx);
            }
        }
    }
    ...
    public function makeTrieTree($sensitiveWords = [],$mgc_lx = null){
        if (!is_string($mgc_lx) || empty($mgc_lx)) {
            throw new \Exception('The loaded mgc type is empty!');
        }

        if(!empty($sensitiveWords) && is_array($sensitiveWords)){
            foreach ($sensitiveWords as $word) {
                if ($word == '') break;
                $now_words = &$this->_wordsTrieTree[$mgc_lx];//才开始判断时，当前词就是整个树
     ...
     * @return int
     */
    private function _check($text, $beginIndex, $length,$mgc_lx)
    {
        $flag = false;
        $c_start = false;
        $word_length = 0;
        $trie_tree = &$this->_wordsTrieTree[$mgc_lx];
     ...
     * @throws
     */
    public function load($sensitiveWords = [],$mgc_lx = null)
    {
        if (!is_string($mgc_lx) || empty($mgc_lx)) {
            throw new \Exception('The loaded mgc type is empty!');
        }
        ...
        $this->makeTrieTree($sensitiveWords,$mgc_lx);
        return true;
    }
    ...
    /* 增加敏感词 */
    public function addMgc($mgc = [],$mgc_lx = null){
        if(empty($mgc)){
            throw new \Exception('添加敏感词不能为空');
        }
        if(!in_array($mgc_lx,$this->_mgclx)){
            throw new \Exception('敏感词类型错误');
        }
        $this->makeTrieTree($mgc,$mgc_lx);
        return true;
    }
    ...
    public function scan_text($text){
        $data = [];
        foreach ($this->_mgclx as $mgc_lx) {
            $scan_result =  $this->scan($text,$mgc_lx);
            $data['sm'][$mgc_lx] = $scan_result;
            $data['words'] = array_unique(array_merge($data['words']??[],$scan_result['words']));
            $data['mgc'] = array_unique(array_merge($data['mgc']??[],$scan_result['mgc']));
        }
        return $data;
    }
     ...
     */
    public function scan($text,$mgc_lx)
    {
        $scan_result = [];
        $ytext = $text;
        ...
        for ($i = 0; $i < $text_length; $i++) {
            $word_length = $this->_check($text, $i, $text_length,$mgc_lx);
